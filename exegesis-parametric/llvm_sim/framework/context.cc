...
std::unique_ptr<const GlobalContext> GlobalContext::Create(llvm::StringRef TripleName, llvm::StringRef CpuName, llvm::StringRef ParametersFilename) {
  return CreateMutable(TripleName, CpuName, ParametersFilename);
}

std::unique_ptr<GlobalContext> GlobalContext::CreateMutable(
    llvm::StringRef TripleName, llvm::StringRef CpuName, llvm::StringRef ParametersFilename) {
  std::string ErrorMsg;
  ...
  Context->Parameters = absl::make_unique<llvm::mca::Parameters>(ParametersFilename);
  ...
  Context->AsmInfo.reset(Target->createMCAsmInfo(
      *Context->RegisterInfo, Context->Triple.str()));
...
void GlobalContext::ComputeInstructionUops(
  ...
  if (Parameters->isPresent()) {
    for (int ProcResIdx = 1; ProcResIdx < SchedModel->getNumProcResourceKinds();
         ++ProcResIdx) {
      const auto &PR = SchedModel->getProcResource(ProcResIdx);

      std::stringstream s;
      s << "port-" << Inst.getOpcode() << "-" << PR->Name;
      auto key = s.str();
      ...
      unsigned timing = Parameters->readInt(key);
      ProcResIdxToCycles[ProcResIdx] = timing;
    }
  } else {
    for (const auto* WPR = SubtargetInfo->getWriteProcResBegin(SCDesc);
         WPR != SubtargetInfo->getWriteProcResEnd(SCDesc); ++WPR) {
      assert(ProcResIdxToCycles[WPR->ProcResourceIdx] == 0 && "WriteProcRes reference the same ProcResIdx multiple times");
  ...
  for (int ProcResIdx = 1; ProcResIdx < SchedModel->getNumProcResourceKinds();
    ...
    std::cout << "Port " << ProcResIdx << "=" << NumCycles << std::endl;
...
void GlobalContext::ComputeUopLatencies(
  ...
  for (int I = 0; I < SCDesc->NumWriteLatencyEntries; ++I) {
    ...
    std::cout << "Latency=" << cyc << std::endl;
    if (cyc > MaxDefLatency) {
      MaxDefLatency = cyc;
    }
  }
...
const InstrUopDecomposition& GlobalContext::GetInstructionDecomposition(
  ...
  std::cout << "Opcode Name=" << InstrInfo->getName(Inst.getOpcode()).data() << std::endl;
