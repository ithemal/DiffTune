...
int main(int argc, char **argv) {
  ...
  Parameters = llvm::make_unique<mca::Parameters>(llvm::ParametersFilename);
  auto Params = std::move(Parameters);

  unsigned Width;
  if (Params->isPresent()) {
    assert(Params->contains("dispatch-width"));
    Width = Params->readInt("dispatch-width");
  } else {
    Width = SM.IssueWidth;
  }
  if (DispatchWidth)
  ...
  mca::InstrBuilder IB(*STI, *MCII, *MRI, MCIA.get(), *Params);
  ...
  for (const std::unique_ptr<mca::CodeRegion> &Region : Regions) {
	...
	std::cout << MCI.getOpcode() << std::endl;
    ...
    auto P = MCA.createDefaultPipeline(PO, IB, S, *Params);
    mca::PipelinePrinter Printer(*P);
