  ...
  for (const auto &MI : *MBB) {
    ...
    if (Parameters->isPresent()) {
      for (unsigned I = 1; I < SchedModel.getNumProcResourceKinds(); I++) {
        PRCycles[I] += readUnrolledCycles(&SchedModel, MI.getOpcode(), I);
      }
    } else {
...
unsigned MachineTraceMetrics::Trace::getResourceLength(
    ...
    ArrayRef<const MachineInstr *> ExtraInstrs,
    ArrayRef<const MachineInstr *> RemoveInstrs) const {
  ...
  auto extraCycles = [this](ArrayRef<const MachineInstr *> Instrs,
                            unsigned ResourceIdx)
                         ->unsigned {
    unsigned Cycles = 0;
    for (const MachineInstr *MI : Instrs) {
      auto SC = TE.MTM.SchedModel.resolveSchedClass(MI);
      // auto SC_idx = TE.MTM.TII->get(MI->getOpcode()).getSchedClass();
      // auto SC = TE.MTM.SchedModel.getMCSchedModel()->getSchedClassDesc(SC_idx);

      if (!SC->isValid())
        continue;

      if (Parameters->isPresent()) {
        Cycles += readUnrolledCycles(&TE.MTM.SchedModel, MI->getOpcode(), ResourceIdx);
      } else {
        TargetSchedModel::ProcResIter PI, PE;
        for (PI = TE.MTM.SchedModel.getWriteProcResBegin(SC),
