...
static uint64_t NormMask(const TargetSchedModel *SchedModel, unsigned I) {
...
static int readUnrolledCycles(const TargetSchedModel *SchedModel, unsigned Opcode, unsigned PortIdx) {
  ...
  for (unsigned I = 1; I <= PortIdx; I++) {
    const MCProcResourceDesc *PR = SchedModel->getProcResource(I);
    std::stringstream s;
    s << "port-" << Opcode << "-" << PR->Name;
    auto key = s.str();

    assert(Parameters->contains(key));
    if ((NormMask(SchedModel, I) & NormMask(SchedModel, PortIdx)) == NormMask(SchedModel, I)) {
      timing += Parameters->readInt(key);
    }
  }
  return timing;
}
...
init(ScheduleDAGMI *DAG, const TargetSchedModel *SchedModel) {
  ...
  for (SUnit &SU : DAG->SUnits) {
    ...
    if (Parameters->isPresent()) {
      for (unsigned I = 1; I < SchedModel->getNumProcResourceKinds(); I++) {
        unsigned Factor = SchedModel->getResourceFactor(I);
        auto cycles = readUnrolledCycles(SchedModel, Opcode, I);
        RemainingCounts[I] += Factor * cycles;
      }
    } else {
...
bool SchedBoundary::checkHazard(SUnit *SU) {
  ...
  if (SchedModel->hasInstrSchedModel() && SU->hasReservedResource) {
    ...
    if (Parameters->isPresent()) {
      for (unsigned ResIdx = 1; ResIdx < SchedModel->getNumProcResourceKinds(); ResIdx++) {
        unsigned Cycles = readUnrolledCycles(SchedModel, SU->getInstr()->getOpcode(), ResIdx);
...
void SchedBoundary::bumpNode(SUnit *SU) {
  ...
  if (SchedModel->hasInstrSchedModel()) {
    ...
    if (Parameters->isPresent()) {
      for (unsigned I = 1; I < SchedModel->getNumProcResourceKinds(); I++) {
        auto cycles = readUnrolledCycles(SchedModel, SU->getInstr()->getOpcode(), I);
        unsigned RCycle = countResource(I, cycles, NextCycle);
        if (RCycle > NextCycle)
          NextCycle = RCycle;
      }
    } else {
    ...
    if (SU->hasReservedResource) {
      ...
      if (Parameters->isPresent()) {
        for (unsigned PIdx = 1; PIdx < SchedModel->getNumProcResourceKinds(); PIdx++) {
          auto cycles = readUnrolledCycles(SchedModel, SU->getInstr()->getOpcode(), PIdx);

          if (SchedModel->getProcResource(PIdx)->BufferSize == 0) {
            if (isTop()) {
              ReservedCycles[PIdx] =
                std::max(getNextResourceCycle(PIdx, 0), NextCycle + cycles);
            }
            else
...
initResourceDelta(const ScheduleDAGMI *DAG,
  ...
  if (Parameters->isPresent()) {
    for (unsigned I = 1; I < SchedModel->getNumProcResourceKinds(); I++) {
      auto cycles = readUnrolledCycles(SchedModel, SU->getInstr()->getOpcode(), I);
      if (I == Policy.ReduceResIdx)
        ResDelta.CritResources += cycles;
      if (I == Policy.DemandResIdx)
        ResDelta.DemandedResources += cycles;
    }
  } else {
    for (TargetSchedModel::ProcResIter
           PI = SchedModel->getWriteProcResBegin(SC),
