...
void DispatchStage::updateRAWDependencies(ReadState &RS,
  ...
  for (WriteRef &WR : DependentWrites) {
    ...
    int ReadAdvance = 0;
    if (!Params.isPresent()) {
     ReadAdvance = STI.getReadAdvanceCycles(SC, RD.UseIndex, WriteResID);
    } else {
	std::stringstream s;
	s << "readadvance-" << RS.getOpcode() << "-" << RD.UseIndex << "-" << WriteResID;
	auto key = s.str();
	ReadAdvance = Params.readInt(key);
    }
    LLVM_DEBUG(dbgs() << "OpCode: " << RS.getOpcode() << " UseIndex: " << RD.UseIndex << " WriteResID: " << WriteResID << " ReadAdvance: " << ReadAdvance << "\n");
