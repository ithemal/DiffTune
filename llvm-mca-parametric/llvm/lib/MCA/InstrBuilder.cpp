...
InstrBuilder::InstrBuilder(const llvm::MCSubtargetInfo &sti,
                           ...
                           const llvm::mca::Parameters &params
                           )
  : STI(sti), MCII(mcii), MRI(mri), MCIA(mcia), Params(params), FirstCallInst(true),
...
static void initializeUsedResources(InstrDesc &ID,
  ...
  if (Params.isPresent()) {
    for (unsigned I = 1; I < NumProcResources; I++) {
      ...
      unsigned timing = Params.readInt(key);
      if (timing > 0) {
        if (PR.BufferSize < 0) {
      ...
      CycleSegment RCy(0, timing, false);
  ...
  if (Params.isPresent()) {
    for (int I = Worklist.size() - 1; I >= 0; --I) {
      ...
      for (int J = I - 1; J >= 0; --J) {
        ...
        uint64_t NormalizedMask = B.first;
        if (countPopulation(B.first) != 1) {
          // Remove the leading 1 from the resource group mask.
          NormalizedMask ^= PowerOf2Floor(NormalizedMask);
        }

        if ((NormalizedMask & A.first) == NormalizedMask) {
          A.second.CS.add(B.second.size());
        }
      }
    }
  }
  ...
  for (unsigned I = 0, E = Worklist.size(); I < E; ++I) {
    ...
    for (unsigned J = I + 1; J < E; ++J) {
      ...
      if ((NormalizedMask & B.first) == NormalizedMask) {
        ...
        if (countPopulation(B.first) > 1) {
          B.second.NumUnits++;
          B.second.NumUnits = std::min(countPopulation(NormalizedMask), B.second.NumUnits);
        }
      }
    }
  }
  ...
  for (unsigned I = 0; I < ID.Resources.size(); I++) {
    if (ID.Resources[I].second.size() == 0) {
      ID.Resources.erase(ID.Resources.begin() + I);
    }
  }
...
int InstrBuilder::getCycles(const MCInstrDesc &MCDesc, const MCSchedClassDesc &SCDesc, unsigned idx) {
  const MCWriteLatencyEntry *WLEntry = STI.getWriteLatencyEntry(&SCDesc, idx);
  std::stringstream s;
  s << "latency-" << MCDesc.Opcode << "-" << idx;
  auto key = s.str();

  auto cyc = 0;
  if (Params.contains(key)) {
    cyc = Params.readInt(key);
  } else {
    cyc = WLEntry->Cycles;
  }
  return cyc;
...
int InstrBuilder::getResourceId(const MCInstrDesc &MCDesc, const MCSchedClassDesc &SCDesc, unsigned idx) {
  const MCWriteLatencyEntry *WLEntry = STI.getWriteLatencyEntry(&SCDesc, idx);

  std::stringstream s;
  s << "resource-" << MCDesc.Opcode << "-" << idx;
  auto key = s.str();

  if (Params.contains(key)) {
    return Params.readInt(key);
  } else {
    return WLEntry->WriteResourceID;
  }
}

void InstrBuilder::computeMaxLatency(InstrDesc &ID, const MCInstrDesc &MCDesc,
                                     const MCSchedClassDesc &SCDesc,
                                     const MCSubtargetInfo &STI) {
  if (MCDesc.isCall()) {
  ...
  int Latency = 0;
  for (unsigned DefIdx = 0, DefEnd = SCDesc.NumWriteLatencyEntries; DefIdx != DefEnd; ++DefIdx) {
    int cycles = getCycles(MCDesc, SCDesc, DefIdx);
    if (cycles < 0) {
      Latency = cycles;
      break;
    }
    Latency = std::max(Latency, static_cast<int>(cycles));
  }
...
void InstrBuilder::populateWrites(InstrDesc &ID, const MCInst &MCI,
  ...
  for (; i < MCI.getNumOperands() && CurrentDef < NumExplicitDefs; ++i) {
    ...
    if (CurrentDef < NumWriteLatencyEntries) {
      int cycles = getCycles(MCDesc, SCDesc, CurrentDef);
      // Conservatively default to MaxLatency.
      Write.Latency = cycles < 0 ? ID.MaxLatency : static_cast<unsigned>(cycles);
      Write.SClassOrWriteResourceID = getResourceId(MCDesc, SCDesc, CurrentDef);
      LLVM_DEBUG({
...
InstrBuilder::createInstrDescImpl(const MCInst &MCI) {
  ...
  if (Params.isPresent()) {
      ...
      ID->NumMicroOps = Params.readInt(key);
  } else {
  ...
  initializeUsedResources(*ID, MCDesc.Opcode, SCDesc, STI, ProcResourceMasks, Params);
...
InstrBuilder::createInstruction(const MCInst &MCI) {
  ...
  bool IsOptimizableRegisterMove = false;
  if (MCIA) {
    unsigned ProcID = STI.getSchedModel().getProcessorID();
    IsZeroIdiom = MCIA->isZeroIdiom(MCI, Mask, ProcID);
    IsDepBreaking =
        MCIA->isDependencyBreaking(MCI, Mask, ProcID);
    IsOptimizableRegisterMove = MCIA->isOptimizableRegisterMove(MCI, ProcID);
    // std::cout << "IsZeroIdiom " << IsZeroIdiom << " IsDepBreaking " << IsDepBreaking << "IsOptimizableRegisterMove " << IsOptimizableRegisterMove << std::endl;
    IsDepBreaking = IsZeroIdiom || IsDepBreaking;
    if (IsOptimizableRegisterMove)
  ...
  for (const ReadDescriptor &RD : D.Reads) {
    ...
    NewIS->getUses().emplace_back(RD, RegID, MCI.getOpcode());
